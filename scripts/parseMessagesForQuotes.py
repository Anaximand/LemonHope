...
def extract_messages(file_path: str) -> list[str]:
    ...
    return [msg.get('content', '') for msg in messages if msg.get('content') and msg.get('content').startswith('Remembered')]
...
def main():
    ...
    for msg in all_content:
        parsed = parse_remembered_message(msg)
        if parsed:
    ...
    if quotes_map and quotes_without_number:
        ...
        for i, quote in enumerate(quotes_without_number):
            if i < len(missing_numbers):
                ...
                print(f"  Warning: No slot available for unnumbered quote: {quote['message'][:50]}...", file=sys.stderr)
